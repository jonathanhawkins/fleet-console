...
describe("service records", () => {
  ...
  it("reads the due figure straight off the interval", () => {
    expect(serviceDueInDays({ servicedDaysAgo: 30, note: "", intervalDays: 180 })).toBe(150);
    expect(serviceDueInDays({ servicedDaysAgo: 180, note: "", intervalDays: 180 })).toBe(0);
  });
  ...
  it("dates a record against the app's clock, not against a hardcoded year", () => {
    ...
    expect(serviceDateLabel({ servicedDaysAgo: 240, note: "", intervalDays: 1 }, at)).toBe(
      "27 Dec 2025",
    );
  });
