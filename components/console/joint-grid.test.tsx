...
import { JOINT_GRID_ORDER } from "./joint-spec";
    ...
    class {
      ...
      observe(el: Element) {
        this.cb(
          [
            {
              contentRect: { width: CELL_W, height: el.tagName === "CANVAS" ? CELL_H : 0 },
...
describe("JointGrid hover", () => {
  ...
  it("does not answer a touch that has not been pressed", () => {
    ...
    fireEvent.pointerMove(canvas, { clientX: xFor(-30), pointerType: "touch", pointerId: 7 });
    expect(currentCursor()).toBeNull();

    fireEvent.pointerDown(canvas, { clientX: xFor(-30), pointerType: "touch", pointerId: 7 });
    expect(currentCursor()).toEqual({ unitId: "N-07", offset: -30, scrubbing: true });

    fireEvent.pointerMove(canvas, { clientX: xFor(-31), pointerType: "touch", pointerId: 7 });
...
describe("TelemetryCursorMeta", () => {
  ...
  it("ignores a cursor on another unit's grid", () => {
    ...
    hover(-40);
    expect(
      document.querySelector("[data-slot='telemetry-cursor-meta']")!.hasAttribute("data-active"),
    ).toBe(false);
  });
});
