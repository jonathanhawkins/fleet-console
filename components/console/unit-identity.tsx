...
import { UnitPostureTag } from "./posture-tag";
import { formatRecency, isoTime, useNow } from "./relative-time";
 ...
 *
 * `selectLastContact` is quantised to a second in the store, so a unit
 * reporting ten times a second re-renders this block once a second — in step
 * with the shared ticker that formats the label, rather than eighty times
 * faster than the text can change.
 */
...
export function UnitIdentity({ className, unitId, ...props }: UnitIdentityProps) {
  ...
  const lastContact = useFleetStore(selectLastContact(unitId));
  const now = useNow();
