...
export function cohortLog(
  ...
  return audit.filter((e) => e.unitId === FLEET_AUDIT_SCOPE && e.ts >= detectedAt);
}
...
export function cohortTimes(
  ...
  const cleared = rows.map((r) => r.cleared).filter((t): t is number => t !== undefined);
...
export function haltRefusal(log: readonly AuditEntry[]): HaltRefusal | null {
  ...
  const reason = at === -1 ? "" : entry.summary.slice(at + FAILED_MARKER.length).trim();
...
export function cohortActions(log: readonly AuditEntry[], fw: string): CohortAction[] {
  ...
  for (const entry of [...log].reverse()) {
    switch (entry.kind) {
      ...
      case "command-failed":
        out.push({
          label: entry.ref?.startsWith("HALT_ROLLOUT")
            ? "Halt rollout"
            : "Roll back cohort",
