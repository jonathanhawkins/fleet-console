...
import { setDescentOccluded } from "@/components/console";
import {
...
const read = (file: string) => readFileSync(join(COMPONENTS, "fleet", file), "utf8");
const readConsole = (file: string) =>
  readFileSync(join(COMPONENTS, "console", file), "utf8");
...
describe("the code-splitting boundary (PRD §7)", () => {
  it("keeps three.js out of everything the unit page's barrel can reach", () => {
    // Both barrels travel with the unit page. Anything they name travels with it.
    expect(read("index.ts")).not.toMatch(/component-scene/);
    expect(readConsole("index.ts")).not.toMatch(/component-scene/);
