...
describe("the document's frame", () => {
  it("says what it is and when it was made", () => {
    ...
    // Date, clock and zone: a stamp of `10:33:53` alone dates the document to
    // no particular day, which is the one thing a filed report cannot do.
    expect(
      screen.getByText(/^Generated .+ \d{4}, \d{2}:\d{2}:\d{2} \S+$/),
    ).toBeInTheDocument();
  });

  it("dates the incident itself, not the moment it was printed", () => {
    renderReport();
    // The letterhead carries the day the chronology below belongs to. Read off
    // the raise, so a copy printed the next morning still says when it happened.
    expect(screen.getByText(/\d{4} · times in \S+$/)).toBeInTheDocument();
  });
  ...
  it("keeps the console's own controls off the printed page", () => {
    renderReport();
    expect(
      screen.getByRole("button", { name: "Close incident report" }).className,
