...
/**
 * Machine view, end to end, against the static WorkerTransport build —
 ...
 *
 * This is the *opt-in* reading of the incident. The product's default walk is
 * golden-path.spec.ts, which takes the same fault through the calm
 * operator-space panel; this one asserts what the descent adds — the page
 * draining, the surface wiping up, the machine chunk arriving only now, and
 * the board's own controls.
 *
 * The preference is set before the first navigation (the beforeEach below), so
 * this spec never has to reach for a toggle: it is describing the console as an
 * operator who has chosen machine space sees it.
 */
